...
double evaluatePostfix(const std::string& postfix) {
    ...
    while(iss >> token) {
        if(isOperator(token[0])) {
            ...
            switch(token[0]) {
                case '+': result = Hometracker::add(a, b); break;
                case '-': result = Hometracker::subtract(a, b); break;
                case '*': result = Hometracker::multiply(a, b); break;
                case '/': 
                    if (b == 0) {
                        throw std::invalid_argument("Division by zero is not allowed.");
                    }
                    result = Hometracker::divide(a, b); break;
            }
